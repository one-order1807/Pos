...
export async function loadState(): Promise<{ state: State; count: number }> {
...
export async function writeChanges(changes: DocChange[]): Promise<void> {
...
export async function getDirty(limit: number): Promise<DirtyRow[]> {
  const d = await openDb();
...
export async function markSynced(rows: DirtyRow[]): Promise<void> {
