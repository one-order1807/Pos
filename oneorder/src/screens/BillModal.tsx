...
import { Btn, Chip, Field, Modal, toast } from '../ui/components';
...
export function BillModal({ sessionId, onClose }: { sessionId: string | null; onClose: () => void }) {
  ...
  function onPay() {
    ...
    if (res.error === 'unsent-items') toast('Send the Cook Bill for new items before payment.', 'error');
    else if (res.error) toast('Could not record payment.', 'error');
    else {
    ...
    <Modal visible onClose={onClose} title="Customer Bill" width={720}>
      <View style={styles.body}>
        ...
        <View style={styles.right}>
          ...
          <Field
            label="Phone (optional, builds loyalty history)"
            value={phone}
          ...
          <Btn label="Payment received — close" icon="check-circle" variant="success" full onPress={onPay} disabled={busy} />
